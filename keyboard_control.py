...
def continuous_forward(motor):
    print("continuou forward")
    motor.set_duty_cycle(0.1)

def continuous_press(key, motor):
    while key == keyboard.Key.right:
        print("continuous forward test")

    while key == keyboard.Key.left:
        print("continuous back test")

            #continuous_forward(motor)
        #else while key == keyboard.Key.left:
            #drive_backward(motor)
        #elif key == keyboard.Key.space:
            #motor.set_duty_cycle(0)
    #     elif key == keyboard.Key.esc:
